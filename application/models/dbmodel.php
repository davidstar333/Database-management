...
class DbModel extends CI_Model {
    ...
    */
    ...
    public function getCell($db, $table, $column, $recordID)
    {
		...
		public function spec_validation($db, $table, $razon, $fact_numo) {
			$tempp = $this->theDB->from($table)->where('Razon_social_proveedor', $razon)->get()->result_array();
			$success = "true";

			foreach( $tempp as $temp ) {
				if( $fact_numo == $temp['Fact_numero'] ) {
					$success = 'false';
					break;
				}
			}
			...
			if($success == 'true') {
				$tempp = $this->theDB->from($table)->where('Fact_numero', $fact_numo)->get()->result_array();
				foreach( $tempp as $temp ) {
					if( $razon == $temp['Razon_social_proveedor'] ) {
						$success = 'false';
						break;
					}
				}
    ...
    public function newRecord($db, $table, $data)
    {
    	...
    	foreach( $data as $key=>$value ) {
    		...
    		if( $key == 'created' ) {
					...
					//unset( $data[$key] );
					$data['created']=date("m/d/y G:i", time());
