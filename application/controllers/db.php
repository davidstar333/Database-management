...
class Db extends CI_Controller {
	...
	*/
	...
	public function saveField($db = '', $table = '', $column = '') 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->data['success_message_heading'] = "Muy bien!";
			$this->data['success_message'] = "La informaci&oacute;n se ha guardado correctamente.";
	...
	*/
	...
	public function spec_validation() {
		...
		$fact_numo = $this->input->post('fact_numo');
		$db = 'app_db';
		$table = 'agregar';
		$this->dbmodel->initialize($db);
		$result = $this->dbmodel->spec_validation($db, $table, $razon, $fact_numo);
