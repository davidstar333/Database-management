...
<body>
	...
	<input type="hidden" name="" id="sess_user" value="<?php echo $this->session->userdata('username'); ?>" />
...
.right-scroll {
    ...
    <span class="scroll right-scroll">&#187;</span>
    			    		<table class="table table-bordered table-striped table-hover <?php if(isset($tableUpdateAllowed) && $tableUpdateAllowed == 'yes'):?>allowedTable<?php else:?>notAllowedTable<?php endif;?>" id="table">
    			    		    <thead>
    			    		    	<tr>
    			    		    		<th width="110px;" style="min-width: 110px!important;"><?php echo $this->lang->line('table_column_actions')?></th>
    			    		    		<?php $colCounter = 0;?>
                                        <?php $user = $this->usermodel->getUser($this->session->userdata('user_id')); $grupo = $user->group_id;

      $rol = $this->rolemodel->getRole2($grupo);
...
<?php foreach($tableFields as $field):?>
                                        ...
                                        <?php endforeach;?>
                                             ...
                                             <?php } else { ?>
                                                <?php foreach($tableFields as $field):?>
<?php if(in_array($theTable.'.'.$field['field'], $permisos)):?>
                                                <th><?php echo $field['field'];?></th>
                                            <?php $colCounter++;?>
                                            <?php endif;?>
...
<script src="<?php echo base_url();?>js/jquery.dataTablesOld.min.js"></script>
...
//});   
 ...
 <?php endforeach;?>
    ...
    document.querySelector('.ze_wrapper').scrollLeft += 140;
}) 
